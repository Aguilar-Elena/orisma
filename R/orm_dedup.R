...
orm_dedup <- function(refs,
  ...
  if (verbose) cli::cli_h1(.msg("phase_dedup", lang))
  if (verbose) cli::cli_alert_info(.msg("dedup_start", lang))
  ...
  if (!"doi" %in% names(refs) || all(is.na(refs$doi))) {
    if (verbose) cli::cli_alert_warning(.msg("err_no_doi", lang))
    n_doi_removed <- 0L
  } else {
    refs <- refs %>%
      dplyr::mutate(
  ...
  if (verbose) {
    cli::cli_alert_success(.msg("dedup_doi", lang, n_removed = n_doi_removed))
  }
  ...
  if (!"title" %in% names(refs) || all(is.na(refs$title))) {
    cli::cli_alert_danger(.msg("err_no_title", lang))
  ...
  if (verbose) {
    cli::cli_alert_success(.msg("dedup_title", lang,
  ...
  if (verbose) {
    cli::cli_alert_success(.msg("dedup_fuzzy", lang,
