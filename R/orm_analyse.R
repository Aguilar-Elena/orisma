...
orm_analyse <- function(mx,
  ...
  if (verbose) {
    cli::cli_h1(.msg("phase_analyse", lang))
    cli::cli_alert_info(.msg("analyse_start", lang))
  }
  ...
  if (!is.null(material_col) && material_col %in% names(refs)) {
    ...
    if (verbose) {
      cli::cli_alert_success(.msg("analyse_mgp", lang, n_mats = n_mats))
    }
